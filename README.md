...
- Velocity not being limited (You fall at a fixed speed, which I hated it, so i removed it)

this will ONLY update if the original is updated, and the update is actually needed. or a bug fix caused by what I did.

i do NOT know java, so if you encounter an error, create an issue for here.
but if it ALSO happens on the original, then create an issue on there
